...
import client.gui.table.renderer.DateCellRenderer;
...
//        });
    ...
    public void tableListener(Bean bean) {

        if (isAllowUpdate()) {
            try {
                modelNewPanel = new ModelNewPanel(bean);
                int ok = DialogUtilities.showOkCancelDialog(getOwner(), modelNewPanel, "Edit");
                if (ok == DialogUtilities.RESULT_OK) {
                    {
                        beanTableModel.updateRow(bean.getModelRow(), modelNewPanel.getRow());
                    }

                }
            } catch (Exception ex) {
                Logger.getLogger(FilterTableFrame.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
    ...
    public boolean isAcceptable() {
        boolean acceptable = modelNewPanel.isAcceptable();
//        if(!acceptable)
//        {
//            MessageUtils.showWarningMessage(getOwner(),  TOOL_TIP_TEXT_KEY);
//        }
    ...
    public void setBeanComplexElement(BeanComplexElement beanComplexElement) {
        this.beanComplexElement = beanComplexElement;
    }
